...
def load_brainvision_vhdr(path):
    ...
    raw.set_channel_types({'EMGi': 'emg'}) 
    print('')
    print('Done!')
...
def filtering(raw,name_channel = 'C4', sf = 250, overlapping = 50, win = 5): #Filering with windows of five seconds and selected overlapping
...
def KCs(filtered_signal,name_channel, path_KC, sf = 250): #Return a vector with only KCs labeled (everything else is zero)
...
def align_KC_list(start, end, data, sf = 250):
    d = 2 #in seconds
    dur = d * sf
    KC = data[0,start:end]
    KC_list = KC.tolist()
...
def getKCs_list(data, name_channel = 'C4', sf = 250, pathKC = ''): 
...
def re_esctructure(raw, filtered_signal, detected, path_KC):
    ...
    new_data[3]= KCs(filtered_signal, 'C4', sf = 250, path_KC=path_KC)
    new_data[4], _ = put_flag(filtered_signal, c4_1, sf = 250, name_channel = 'C4', path_KC = path_KC)
    new_data[5]= detected
...
def main():  # Wrapper function
    ...
    #'C:/Users/aylin/PFC/Experimentacion_nuestra/Scoring/ancarosi1.txt'#
    raw_0,_ = set_sleep_stages(raw,path_states)
   
    path_KC1 = easygui.fileopenbox(title='Select the labeling file (with extension txt).')
    #'C:/Users/aylin/PFC/Experimentacion_nuestra/KC ETIQUETADOS/ancarosi1_2021March09_18-48.txt' #
    
    filtered_signal = filtering(raw_0, sf = 250, name_channel='C4')

    list_test = getKCs_list(filtered_signal, name_channel='C4', sf = 250, pathKC = path_KC1)
   ...
   ########################################################################################################################
    ...
    pos_c4 =(raw_0.ch_names).index('C4')
    ...
    detected, cant_KC = detector(clf,filtered_signal,candidates, sf = 250)    
